...
import (
	...
	"goFoundation/webook/interactive/integration/startup"
...
type InteractiveTestSuiteGrpc struct {
	suite.Suite
	db  *gorm.DB
	rdb redis.Cmdable
	svc *grpc.ServerGrpc
...
func (s *InteractiveTestSuiteGrpc) SetupSuite() {
	s.db = startup.InitTestDB()
	s.rdb = startup.InitRedis()
	s.svc = startup.InitInteractiveGRPCServer()
}

func (s *InteractiveTestSuiteGrpc) TearDownTest() {
...
func (s *InteractiveTestSuiteGrpc) TestIncrReadCnt() {
	testCases := []struct {
		name    string
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			intr, err := s.svc.IncrReadCnt(context.Background(), &intrv1.IncrReadCntRequest{
				Biz:   tc.biz,
				BizId: tc.bizId,
			})
			assert.Equal(t, tc.wantErr, err)
			assert.Equal(t, tc.wantRes, intr)
			tc.after(t)
		})
	}
}

func (s *InteractiveTestSuiteGrpc) TestLike() {
	t := s.T()
	testCases := []struct {
		name    string
		before  func(t *testing.T)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			intr, err := s.svc.Like(context.Background(), &intrv1.LikeRequest{
...
func (s *InteractiveTestSuiteGrpc) TestDislike() {
	t := s.T()
	testCases := []struct {
		name    string
		before  func(t *testing.T)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			intr, err := s.svc.CancelLike(context.Background(), &intrv1.CancelLikeRequest{
				Biz: tc.biz, BizId: tc.bizId, Uid: tc.uid,
			})
			assert.Equal(t, tc.wantErr, err)
			assert.Equal(t, tc.wantRes, intr)
...
func (s *InteractiveTestSuiteGrpc) TestCollect() {
	testCases := []struct {
		name    string
		before  func(t *testing.T)
		after   func(t *testing.T)
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			intr, err := s.svc.Collect(context.Background(), &intrv1.CollectRequest{
...
func (s *InteractiveTestSuiteGrpc) TestGet() {
	testCases := []struct {
		name    string
		before  func(t *testing.T)
		bizId   int64
...
func (s *InteractiveTestSuiteGrpc) TestGetByIds() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			ids:  []int64{1, 2},
			wantRes: &intrv1.GetByIdsResponse{
				Intr: map[int64]*intrv1.Interactive{
					1: {
						BizId:      1,
						ReadCnt:    1,
						CollectCnt: 2,
...
func TestInteractiveServiceGrpc(t *testing.T) {
	suite.Run(t, &InteractiveTestSuiteGrpc{})
}
